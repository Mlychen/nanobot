...
def test_agent_uses_config_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    config_file = tmp_path / "instance" / "config.json"
    config_file.parent.mkdir(parents=True)
    config_file.write_text("{}")
    ...
    monkeypatch.setattr("nanobot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("nanobot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("nanobot.identity.IdentityStore", lambda _workspace: object())
    monkeypatch.setattr("nanobot.identity.IdentityMapper", lambda _store: MagicMock(enrich_metadata=lambda *_args, **_kwargs: {}))

    class _FakeResolvedSession:
        session_key = "cli:direct"

    monkeypatch.setattr(
        "nanobot.routing.SessionPolicy",
        lambda _store: MagicMock(
            resolve_session_key=lambda **_kwargs: _FakeResolvedSession(),
            enrich_metadata=lambda base, _resolved: base,
        ),
    )
    monkeypatch.setattr("nanobot.routing.SessionPolicyStore", lambda _workspace: object())
    monkeypatch.setattr("nanobot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_gateway_uses_configured_port_when_cli_flag_is_missing(
